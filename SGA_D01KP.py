...
class SGA_D01KP(object):
    def __init__(self, path1, path2, KP_C, iteration=500, person_num=30, ga_choose_ratio=0.2, cross_ratio=0.8,
                 mutate_ratio=0.1):
    ...
    def NROA(self, onePerson):  # NROA算法对整形向量进行贪心修复
        fweight = 0
        fvalue = 0
        for i in range(self.KP_N):  # 将非正常编码个体修复为正常编码个体
            k = math.floor(self.KP_H[i] / 3)
            r = self.KP_H[i] % 3
            if (onePerson[k] == r + 1) and ((fweight + self.KP_W[self.KP_H[i]]) <= self.KP_C):
                fweight = fweight + self.KP_W[self.KP_H[i]]
                fvalue = fvalue + self.KP_P[self.KP_H[i]]
            elif (onePerson[k] == r + 1) and ((fweight + self.KP_W[self.KP_H[i]]) > self.KP_C):
                onePerson[k] = 0
        for i in range(self.KP_N):  # 对个体进一步优化处理
            k = math.floor(self.KP_H[i] / 3)
            r = self.KP_H[i] % 3
            if (onePerson[k] == 0) and ((fweight + self.KP_W[self.KP_H[i]]) <= self.KP_C):
                fweight = fweight + self.KP_W[self.KP_H[i]]
                fvalue = fvalue + self.KP_P[self.KP_H[i]]
                onePerson[k] = r + 1
        return onePerson, fvalue

    def initialPopolation(self):  # 初始化种群
        total_population = []
        p = 0.7  # 设置生成除0以外的概率，这个可以调整，以达到最优
        while len(total_population) != self.person_num:
            person = []
            for i in range(int(self.KP_N/3)):
                if random.random() < p:
                    person.append(np.random.randint(1,4))
                else:
                    person.append(0)
    ...
    def ga_mutate(self, gene):
        ...
        order = list(random.sample(path_list, int(len(gene) * 0.1)))
        for i in order:
            temp = gene[i]
            if np.random.uniform(0, 1) < self.mutate_ratio:
                while gene[i] == temp:
                    gene[i] = np.random.randint(0, 4)
        return gene
    ...
    def run(self):  # 开始迭代
        ...
        for i in range(1, self.iteration + 1):
            ...
            for j in range(int(self.person_num)):  # 修复个体并求得最优值
                self.population[j], tmp_fval = self.NROA(self.population[j])
...
if __name__ == '__main__':
    model = SGA_D01KP('data/WDKP/WDKP5p.csv', 'data/WDKP/WDKP5w.csv', 326793, 100, 50, 0.2, 0.2, 0.01)
